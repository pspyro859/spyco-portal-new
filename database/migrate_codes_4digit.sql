-- Migration Script: Regenerate Supplier Codes with 4 Letters + 4 Numbers
-- Format: XXXX-XXXX where XXXX = first 4 letters of supplier name

-- Step 1: Backup existing codes (create a backup table)
DROP TABLE IF EXISTS suppliers_codes_backup_5digit;
CREATE TABLE suppliers_codes_backup_5digit AS
...
SELECT 
    ...
    'Codes backed up to suppliers_codes_backup_5digit table' as message
FROM suppliers_codes_backup_5digit;
...
SELECT 
    ...
    'Codes regenerated with 4 letters + 4 digits format' as message
FROM suppliers 
WHERE status != 'deleted';
...
-- Note: If you need to rollback to 5-digit format, use:
-- UPDATE suppliers s JOIN suppliers_codes_backup_5digit b ON s.id = b.id SET s.code = b.code;